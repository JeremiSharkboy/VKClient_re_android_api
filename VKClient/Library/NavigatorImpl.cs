...
namespace VKClient.Library
{
        ...
        private readonly List<NavigatorImpl.NavigationTypeMatch> _navTypesList = new List<NavigatorImpl.NavigationTypeMatch>()
    {
      ...
      new NavigatorImpl.NavigationTypeMatch(NavigatorImpl._namedObjReg, NavigatorImpl.NavType.namedObject)
        ...
        {
            userOrGroupId = userOrGroupId != 0L ? userOrGroupId : AppGlobalStateManager.Current.LoggedInUserId;
            string str = string.Format("/VKClient.Audio;component/AudioPage2.xaml?PageMode={0}&UserOrGroupId={1}&IsGroup={2}&AlbumId={3}&ExcludeAlbumId={4}&AlbumName={5}", pickMode, userOrGroupId, isGroup, albumId, excludeAlbumId, Extensions.ForURL(albumName));
            if (pickMode != 0)
                str += "&IsPopupNavigation=True";
            return str;
        }
        ...
        private static string GetNavigateToVideoWithCommentsString(VKClient.Common.Backend.DataObjects.Video video, long ownerId, long videoId, string accessKey = "")
        {
            ...
            return string.Format("/VKClient.Video;component/VideoCommentsPage.xaml?OwnerId={0}&VideoId={1}&AccessKey={2}&VideoSource={3}&VideoContext={4}", ownerId, videoId, Extensions.ForURL(accessKey), CurrentMediaSource.VideoSource, Extensions.ForURL(CurrentMediaSource.VideoContext));
        }
        ...
        {
            this.Navigate("/VKClient;component/LoginPage.xaml"/*"/VKClient;component/WelcomePage.xaml"*/);
        }

        private void Navigate(string navStr)
        {
            Logger.Instance.Info("Navigator.Navigate, navStr={0}", navStr);
