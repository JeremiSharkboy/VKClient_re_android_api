    ...
    public class GroupProfileInfoViewModel : ProfileInfoViewModelBase
    {
        ...
        public override List<InfoListItem> InfoItems
        {
            get
            {
                ...
                if (!string.IsNullOrEmpty(this._group.Activity) || this._group.AdminLevel > 1)
                {
                    StatusInfoListItem statusInfoListItem1 = new StatusInfoListItem((IProfileData)this._group);
                ...
                if (group.members_count > 0)
                {
                    ...
                    infoListItem1.TapAction = action;
                    InfoListItem infoListItem2 = infoListItem1;
                    List<User> friends = this._group.friends;
                    if (friends != null)
                    {
                        if (friends.Count > 0)
        ...
        public GroupProfileInfoViewModel(Group group)
        {
            this._group = group;
        }
        ...
        {
            if (this._group == null || this._group == null || (this._group.GroupType != GroupType.Event || this._group.start_date == 0))
                return "";
            DateTime dateTime = VKClient.Common.Utils.Extensions.UnixTimeStampToDateTime((double)Math.Max(this._group.start_date, this._group.finish_date), true);
        ...
        private InlinesCollection ComposeInlinesForMembers()
        {
            ...
            run2.Text = str2;
            Run run3 = run2;
            if (this._group.friends != null && this._group.friends.Count > 0)//
            {
                Run run4 = run3;
                ...
                Run run6 = new Run();
                string str5 = UIStringFormatterHelper.FormatNumberOfSomething(this._group.friends.Count, CommonResources.OneFriendFrm, CommonResources.TwoFourFriendsFrm, CommonResources.FiveFriendsFrm, false, null, false);
                run6.Text = str5;
        ...
        {
            Group group = this._group;
            if (string.IsNullOrEmpty(group != null ? group.wiki_page : null))
                return;
            Navigator.Current.NavigateToWebUri(this._group.wiki_page, true, false);
        }
        ...
        public override void OpenLink()
        {
            ...
            if (string.IsNullOrEmpty(str))
                return;
            Navigator.Current.NavigateToWebUri(this._group.site, true, false);
        }
    }
}
