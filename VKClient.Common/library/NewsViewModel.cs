...
namespace VKClient.Common.Library
{
    public class NewsViewModel : ViewModelBase, ICollectionDataProvider<NewsFeedData, IVirtualizable>, IHandle<WallPostAddedOrEdited>, IHandle, IHandle<AdReportedEvent>, IHandle<NewsfeedTopEnabledDisabledEvent>
    {
        ...
        private NewsFeedSectionsAndLists _sectionsAndLists = new NewsFeedSectionsAndLists();
        ...
        //
        public ObservableCollection<StoryAsNews> Stories { get; private set; }
        public class StoryAsNews
        {
        ...
        public Visibility StoriesVisible
        {
        ...
        public Func<NewsFeedData, ListWithCount<IVirtualizable>> ConverterFunc
        {
            get
            {
                return (Func<NewsFeedData, ListWithCount<IVirtualizable>>)(newsFeedData =>
                {
                    ...
                    if (newsFeedData.sections != null)
                        this._sectionsAndLists.sections = newsFeedData.sections;
                    //
                    if (newsFeedData.lists != null)
                        this._sectionsAndLists.lists = newsFeedData.lists;
                    if (newsFeedData.stories != null)
                    {
                        this.Stories.Clear();
                        foreach (List<Story.StoryData> s in newsFeedData.stories.items)
                        {
                            User owner = newsFeedData.stories.profiles.First<User>((u) => u.id == s[0].owner_id);
                            StoryAsNews n = new StoryAsNews();
                            n.owner = owner.first_name;
                            n.owner_photo = owner.photo_100;
                            this.Stories.Add(n);
                        }
                        this.NotifyPropertyChanged<ObservableCollection<StoryAsNews>>(() => this.Stories);
                        this.NotifyPropertyChanged<Visibility>(() => this.StoriesVisible);
        ...
        public bool AreFreshNewsUpToDate
        {
            get
            {
                ...
                if ((DateTime.Now - this._lastFreshNewsCheckDateTime).TotalSeconds >= valueOrDefault)
                    return false;
        ...
        public NewsViewModel()
        {
            ...
            //
            this.Stories = new ObservableCollection<StoryAsNews>();
        }
        ...
        private static UserNotification TryGetBubbleNewsfeedNotification(NewsFeedData newsFeedData)
        {
            ...
            if (newsFeedData.notifications == null)
                return null;
            if (newsFeedData.notifications.items == null)
                return null;
            return (UserNotification)Enumerable.FirstOrDefault<UserNotification>(newsFeedData.notifications.items, (Func<UserNotification, bool>)(n =>
            {
        ...
        private ListWithCount<IVirtualizable> ConvertToVirtualizableList(NewsFeedData newsFeedData)
        {
            ...
            try
            {
                while (enumerator3.MoveNext())
                {
                    ...
                    usersAndGroupsInfo1.Profiles = newsFeedData.profiles;
                    usersAndGroupsInfo1.Groups = newsFeedData.groups;
