...
namespace VKClient.Common.Library
{
    ...
    {
        public static List<PickableItem> DoNotDisturbOptions = new List<PickableItem>() { new PickableItem() { ID = 1L, Name = string.Format(CommonResources.OneHourFrm, "1") }, new PickableItem() { ID = 2L, Name = string.Format(CommonResources.TwoFourHoursFrm, "2") }, new PickableItem() { ID = 3L, Name = string.Format(CommonResources.TwoFourHoursFrm, "3") }, new PickableItem() { ID = 5L, Name = string.Format(CommonResources.FiveHoursFrm, "5") }, new PickableItem() { ID = 8L, Name = string.Format(CommonResources.FiveHoursFrm, "8") } };
        ...
        public string TempDisabledString
        {
            get
            {
                if (this.TempDisabled)
                    return string.Concat(CommonResources.Settings_DisabledNotifications, " ", (AppGlobalStateManager.Current.GlobalState.PushNotificationsBlockedUntil + (DateTime.Now - DateTime.UtcNow)).ToShortTimeString());
                return "";
            }
        }
