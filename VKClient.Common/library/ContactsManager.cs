    ...
    public class ContactsManager : IHandle<FriendRequestAcceptedDeclined>, IHandle, IHandle<FriendRemoved>
    {
        ...
        public async Task EnsureInSyncAsync(bool forceNow = false)
        {
            await ThreadPool.RunAsync((WorkItemHandler)(async o =>
            {
                ...
                if (!loggedInUser)
                    return;
                if (!((System.DateTime.UtcNow - (await this.GetSavedList()).SyncedDate).TotalHours >= 24.0 | forceNow))
                    return;
                UsersService.Instance.GetFriendsAndLists((Action<BackendResult<AllFriendsList, ResultCode>>)(async res =>
                {
                    if (res.ResultCode != ResultCode.Succeeded)
                        return;//todo:bug
                    FriendsCache.Instance.SetFriends(res.ResultData.items, null/*res.ResultData.profiles*/);
                    if (!syncContacts)
                        return;
                    await this.SyncContactsAsync(res.ResultData.items);
                }));
            }));
        }
